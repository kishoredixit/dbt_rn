{{ config( materialized='table') }} with recp_account as
    (SELECT {{ md5_surrogatekey('Id') }} AS "dim_account_key",
                                       "Id" AS "accountid",
     ...
     WHERE executiondate > ( select max(executiondate) from "dwh"."d_account" )     
),
                                                                                     final as
    ( select *
     from recp_account)
