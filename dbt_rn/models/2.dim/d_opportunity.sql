{{ config( materialized="incremental", schema="dwh", Unique_id='opportunityid', post_hook=" delete from {{ this }} where opportunityid in (select opportunityid FROM {{ ref('recp_opportunity') }} where isdeleted = 'y')" ) }} with final as
    (SELECT {{ md5_surrogatekey('id') }} AS "dim_opportunity_key",
                                       "id" as "opportunityid",
     ...
     inner join {{ref('d_account')}} ac on op.accountid=ac.accountid)
select *
from final